...
cli.responders.stats = function () {
    ...
    const stats = {
        ...
        'Current Malloced Memory': `${v8.getHeapStatistics().malloced_memory} bytes`,
        'Peak Malloced Memory': `${v8.getHeapStatistics().peak_malloced_memory} bytes`,
        'Allocated Heap Used (%)': ((v8.getHeapStatistics().used_heap_size / v8.getHeapStatistics().total_heap_size) * 100).toFixed(2),
        'Available Heap Allocated (%)': ((v8.getHeapStatistics().total_heap_size / v8.getHeapStatistics().heap_size_limit) * 100).toFixed(2),
    ...
    // Show each stat, followed by its value, in yellow and white respectively
            ...
            userIds.forEach((userId) => {
                _data.read('users', userId, function (err, userData) {
                    if (!err && userData) {
                        ...
                        const numberOfChecks = userData.checks instanceof Array ? userData.checks.length : 0;
                        line += numberOfChecks;
            ...
            checkIds.forEach((checkId) => {
                _data.read('checks', checkId, function (err, checkData) {
                    if (!err && checkData) {
                        ...
                        if (lowerCaseStr.includes(`--${state}`) || (!lowerCaseStr.includes('--up') && !lowerCaseStr.includes('--down'))) {
                            const line = `ID: ${checkData.id} ${checkData.method.toUpperCase()} ${checkData.protocol}://${
                                checkData.url
...
cli.responders.moreCheckInfo = function (str) {
    // Get check id from the provided string
    const arr = str.split('--');
    const checkId = arr.length > 1 && typeof arr[1] === 'string' ? arr[1].trim() : '';
    if (checkId) {
        // Look up the check
        _data.read('checks', checkId, function (err, checkData) {
            if (!err && checkData) {
    ...
    ls.stdout.on('data', (dataObj) => {
        ...
        const logFilenames = dataStr.split('\n').map((filename) => {
            ...
            if (filename.includes('.log')) {
                trimmedFilename = filename.replace('.log', '');
            } else if (filename.includes('.gz.b64')) {
                trimmedFilename = filename.replace('.gz.b64', '');
            }
        ...
        logFilenames.forEach((logFilename) => {
            // Only display the compressed logs
...
cli.responders.moreLogInfo = function (str) {
    ...
    const arr = str.split('--');
    const logFilename = arr.length > 1 && typeof arr[1] === 'string' ? arr[1].trim() : '';
    if (logFilename) {
        // Decompress the log file
        _logs.decompress(logFilename, function (err, logData) {
            if (!err && logData) {
                cli.verticalSpace();
                const logStrs = logData.split('\n');
                logStrs.forEach((logStr) => {
                    // Parse the string to json object
                    const logObj = helpers.parseJsonToObject(logStr);
