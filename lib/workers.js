        ...
        if (!err && checks && checks.length > 0) {
            checks.forEach(function (check) {
                ...
                _data.read('checks', check, function (err, originalCheckData) {
                    if (err) {
                        debuglog("Error reading one of the checks' data");
                    } else {
            ...
            debuglog('Error: could not find any checks to process');
        }
    });
};
...
workers.processCheckOutcome = function (originalCheckData, checkOutcome) {
    ...
    _data.update('checks', newCheckData.id, newCheckData, function (err) {
        if (err) {
            debuglog('Error trying to save updates to one of the checks');
        } else {
            // Send the new check data to the next phase in the process if needed
            if (alertWarranted) {
                workers.alertUserToStatusChange(newCheckData);
            } else {
                debuglog('Check outcome has not changed, no alert needed');
            }
        }
    });
};
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
        if (err) {
            debuglog('Error: Could not send sms alert to user who had a status change in their check');
        } else {
            debuglog('Success: User was alerted to a status change in their check, via sms: ', msg);
        }
    });
};
...
workers.log = function (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
    ...
    _logs.append(logFilename, logString, function (err) {
        if (err) {
            debuglog('Logging to file failed');
        } else {
            debuglog('Logging to file succeeded');
        }
    });
};
        ...
        if (!err && logs && logs.length > 0) {
            logs.forEach(function (logName) {
                ...
                _logs.compress(logId, newFileId, function (err) {
                    if (err) {
                        debuglog('Error compressing one of the log files', err);
                    } else {
                        // Truncate the log
                        _logs.truncate(logId, function (err) {
                            if (err) {
                                debuglog('Error truncating log file');
                            } else {
                                debuglog('Success truncating log file');
                            }
                        });
                    }
                });
            ...
            debuglog('Error: could not find any logs to rotate');
        }
    });
};
